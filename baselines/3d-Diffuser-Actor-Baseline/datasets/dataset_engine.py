...
class RLBenchDataset(Dataset):
    ...
    def __init__(
        ...
        # Collect all episodes in the dataset
        self._episodes = []
        self._num_episodes = 0
        tasks = os.listdir(self._root)
        for task_str in tasks:
            data_dir = root / task_str
            if not data_dir.is_dir():
                print(f"Can't find dataset folder {data_dir}")
                continue
            npy_episodes = [(task_str, ep) for ep in data_dir.glob("*.npy")]
            dat_episodes = [(task_str, ep) for ep in data_dir.glob("*.dat")]
            pkl_episodes = [(task_str, ep) for ep in data_dir.glob("*.pkl")]
    ...
    def __getitem__(self, episode_id):
        ...
        task, file = self._episodes[episode_id]
        ...
        instr = torch.stack([episode[6][i] for i in frame_ids], dim=0)
