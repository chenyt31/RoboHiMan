...
def get_current_timestep(episode_keypoints, subgoal_keypoints, sample_frame):
    ...
    # 检查next_subgoal_keypoints_idx的奇偶性来确定start_idx
    if next_subgoal_keypoints_idx % 2 == 0:  # 偶数索引
        start_idx = 0
    else:  # 奇数索引
        # 获取前一个子目标关键点的位置
        prev_subgoal = subgoal_keypoints[next_subgoal_keypoints_idx - 1]
        # 找到该子目标在episode关键点中的位置并加1
        start_idx = episode_keypoints.index(prev_subgoal) + 1
        ...
        for d_idx in range(start_idx, start_idx + num_demos):
            ...
            with open(varation_descs_pkl_file, "rb") as f:
                ...
                if train_mode == "vanilla":
                    ...
                    descs = descs['oracle_half'][0].split('\n')
                else:
                    desc = descs[0]
